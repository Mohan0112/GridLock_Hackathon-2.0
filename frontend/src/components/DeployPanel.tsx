...
import { api } from "../api";
import { TIME_BANDS, fmt1 } from "../lib/viz";
...
export default function DeployPanel({ beats, onPlan, onFly }: Props) {
  ...
  async function generate() {
    setBusy(true);
    try {
      const res = await api.beatPlan(teams, band, null);
      onPlan(res.plan);
    } catch {
      onPlan([]);
    } finally {
  ...
  function exportCsv() {
    ...
    link.href = url;
    link.download = `gridlock-beat-plan-${band}.csv`;
    link.click();
    ...
    <div className="space-y-3.5">
      <div>
        ...
        <p className="mt-1.5 text-[13px] leading-relaxed text-paper/90">
          ...
          this shift - kept spatially spread so beats do not overlap.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-line bg-panel/80 p-3.5">
        ...
        <div className="mb-3">
          <div className="mb-1 text-[10px] text-mist">Shift</div>
          <div className="grid grid-cols-5 gap-1">
            {TIME_BANDS.map((b) => (
              <button
                key={b}
                onClick={() => setBand(b)}
                className={`rounded-md border px-1 py-1.5 text-[11px] capitalize transition ${
                  band === b
                    ? "border-amber bg-amber/15 text-amber"
                    : "border-line bg-panel2/50 text-mist hover:border-mist/40"
                }`}
              >
                {b}
              </button>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <div className="text-[10px] text-mist">
              {beats.length}-beat plan / {band} shift
            </div>
            <button
              onClick={exportCsv}
